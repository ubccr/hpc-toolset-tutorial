...
sudo -u pcp /usr/bin/env PMLOGGER_EPILOG=yes pmlogger -c /etc/pcp/pmlogger/pmlogger-supremm.config -s 1 -l /tmp/job-$SLURM_JOB_ID-end-$jobdatelong.log $logdir/job-$SLURM_JOB_ID-end-$jobdatelong > /dev/null 2>&1
